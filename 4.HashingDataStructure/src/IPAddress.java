 ...
 * class IPAddress
 * Purpose:
 *      This class represents the four octets and subnet mask of an IP address
 * Author: Linda Crane
 * Student: Andressa Machado [40923007]
 * data members:
 ...
 *   calcHash       - Return a possible index to be used in the addEntryToTable() in the Router class.
 *                    This method used a hash algorithm to calculate. It sums all the 4 octets and module it to 100
 *                    to make sure it not pass the limit of 99 indexes.
 ***************************************************************************************/
class IPAddress {
    ...
    public int calcHash(int size) {
        IPAddress network = getNetwork();
        int sumOfOctets = 0;

        for (int i = 0; i < network.address.length; i++) {
            sumOfOctets += network.address[i];
        }

        return sumOfOctets % size;
    }
}
